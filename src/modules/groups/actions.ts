...
export async function requestToJoin(groupId: string, formData: FormData) {
  const user = await requireUser();
...
export async function promoteCoLeader(groupId: string, targetUserId: string) {
  const user = await requireUser();
...
export async function demoteCoLeader(groupId: string, targetUserId: string) {
  const user = await requireUser();
  await requireLeader(groupId, user.id);
...
export async function transferLeadership(groupId: string, targetUserId: string) {
  const user = await requireUser();
  const leader = await requireLeader(groupId, user.id);
...
export async function updateProjectDetails(groupId: string, formData: FormData) {
  const user = await requireUser();
...
export async function updateProjectStatus(groupId: string, status: "UPCOMING" | "IN_PROGRESS" | "COMPLETED") {
  const user = await requireUser();
  await requireManager(groupId, user.id);
...
export async function createTask(groupId: string, formData: FormData) {
  const user = await requireUser();
  await requireManager(groupId, user.id);
...
export async function updateTaskStatus(taskId: string, status: "NOT_STARTED" | "IN_PROGRESS" | "COMPLETED") {
  const user = await requireUser();
...
export async function addResource(groupId: string, formData: FormData) {
  const user = await requireUser();
...
export async function startDeletionVote(groupId: string) {
  const user = await requireUser();
  await requireLeader(groupId, user.id);
 ...
 */
export async function castDeletionVote(voteId: string, choice: "DELETE" | "KEEP") {
  const user = await requireUser();
