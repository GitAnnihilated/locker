...
import { RESOURCE_TYPE_META } from "@/modules/groups/meta";
import { removeClassResource } from "../actions";
import type { MyResource } from "../queries";

export function ClassResourceItem({
  resource,
  viewerId,
  showCourse = false,
}: {
  ...
  const canRemove = resource.uploaderId === viewerId;
    ...
    <div className="flex items-start gap-3 border-b border-border px-4 py-3 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-xs text-subtle">
          ...
          {showCourse && "class" in resource && resource.class ? ` · ${resource.class.name}` : ""} ·{" "}
      ...
      {canRemove && (
        <Button
          ...
          onClick={() =>
            start(async () => {
              const result = await removeClassResource(resource.id);
